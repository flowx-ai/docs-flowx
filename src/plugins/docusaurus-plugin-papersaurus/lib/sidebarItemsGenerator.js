...
const DefaultSidebarItemsGenerator = async function defaultSidebarItemsGenerator({ item, docs: allDocs, version, numberPrefixParser, options, }) {
    ...
    function createDocSidebarItem(doc) {
        return Object.assign(Object.assign({ type: 'doc', id: doc.id }, (doc.frontMatter.sidebar_label && {
    ...
    async function createCategorySidebarItem({ breadcrumb, }) {
        ...
        return Object.assign({ type: 'category', label: (_d = categoryMetadatas === null || categoryMetadatas === void 0 ? void 0 : categoryMetadatas.label) !== null && _d !== void 0 ? _d : filename, items: [], collapsed,
            collapsible }, (typeof position !== 'undefined' && { position }));
    }
...
function sortSidebarItems(sidebarItems) {
    const processedSidebarItems = sidebarItems.map((item) => {
        if (item.type === 'category') {
            return Object.assign(Object.assign({}, item), { items: sortSidebarItems(item.items) });
        }
        return item;
    });
    const sortedSidebarItems = (0, lodash_1.orderBy)(processedSidebarItems, (item) => item.position, ['asc']);
    return sortedSidebarItems.map((_a) => {
        var { position: _removed } = _a, item = __rest(_a, ["position"]);
        return item;
    });
}
