...
6. Add a [**User Task**](../../../building-blocks/node/user-task-node) node and configure an [**Upload File**](../../../building-blocks/node/task-node/upload-file-action.md) action - the action will send the file on the topic (defined at the **`KAFKA_TOPIC_DOCUMENT_PERSIST_IN`**) to the [Min.io](https://min.io/) - storage solution.
