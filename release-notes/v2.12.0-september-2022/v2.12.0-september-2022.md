...
The new search data microservice enables you to search/look for data (using [Kafka send](../../docs/building-blocks/node/message-send-received-task-node#configuring-a-message-send-task-node) / [Kafka receive](../../docs/building-blocks/node/message-send-received-task-node#configuring-a-message-receive-task-node)) actions in another processes. Using elastic search the search microservice can look for keys indexed there via existing mechanics.
...
#### Card UI element
 ...
 A new "CARD" UI component is now available to improve the mobile user experience when creating processes with FLOWX.AI. The new card element can be added to a user task using the [UI Designer](../../docs/building-blocks/ui-designer). It has the option to enable/disable an accordion element.

:::tip
The Bootstrap accordion is a component that organizes content within collapsible items. Accordion allows the display of only one collapsed item at a time.
:::
