...
Import BPMN is a feature that helps you edit/create a process starting from an already designed BPMN diagram. You can now import a BPMN process  (**.bpmn** extension) in [FLOWX Designer](broken-reference).

![](../img/release2.8.1_import_bpmn.gif)

:::caution
**NOTE:** Imported processes do not contain [actions](../../../flowx-elements/node/actions.md). You must define and add actions to the process after the import.
:::
...
For more information about UI Designer, check the following section:

[Configure a template config element](../../../flowx-elements/configure-a-template-config-element/)
...
* fixed an issue where getting output from [DMN rule](../../../flowx-elements/node/nodes-types/task-node/business-rule-action/dmn-business-rule-action) would save it as a flatten key
* fixed an issue where token reset values with copy keys would copy the keys as flatten

More information about flatten/unflatten, [here](https://docs.flowx.ai/flowx-elements/node/nodes-types/task-node/business-rule-action#flattened-vs-unflattened-keys).
